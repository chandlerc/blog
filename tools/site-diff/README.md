...
| **Images**    | Bytes are hashed, along with intrinsic size, rendered size and `alt`. Every `srcset` candidate is hashed, not only the one this viewport selects, since the others never appear in a screenshot. Identity is the content: an image or variant renamed to a new path with the same bytes is not a finding. Cross-origin images are blocked, so only their URLs are compared. |
| **Head**      | An allowlist: canonical, RSS autodiscovery, icons, manifest, viewport, robots, charset, `<title>` and `lang`. Not the whole head, because production builds emit OpenGraph and schema.org metadata that development builds do not. Icons are compared by the bytes they serve, so a renamed icon with identical content is not a finding.                                   |
...
| **404**       | One deliberately nonexistent URL is compared alongside the sitemap's routes. The 404 page appears in no sitemap and nothing links to it, so without this a deployment serving a different 404 rendering -- or answering 200 to nonsense -- would be invisible to every other check.                                                                                         |
...
- Third-party requests are blocked. The YouTube embeds in a couple of decks
  ...
  `--allow-third-party` renders them at the cost of that determinism. The other
  origin in the comparison is not a third party: production markup writes this
  site's own URLs absolutely, so requests and references to the counterpart are
  treated as the page's own -- requests are served from the origin under test,
  the way production serving would, and URLs are compared with either origin
  stripped.
